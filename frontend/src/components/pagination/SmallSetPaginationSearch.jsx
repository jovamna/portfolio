import {BellIcon} from '@heroicons/react/24/outline'
import { useState } from 'react';


function SmallSetPaginationSearch({list_page, list, count, term}){
  console.log("SmallSetPaginationSearch Props: ", { list_page, list, count, term }); // Agrega esto para depurar las propiedades
  // Resto de tu componente

  const [active, setActive] = useState(1);
  const [listingsPerPage, setListingsPerPage] = useState(4);
  const [currentPage, setCurrentPage] = useState(1);

  const visitPage = (page) => {
      window.scrollTo(0, 0);
      setCurrentPage(page);
      setActive(page);
      list_page(term,page)
  }

  const previous_number = () => {
    window.scrollTo(0, 0);
    
      if (currentPage !== 1) {
          setCurrentPage(currentPage-1);
          setActive(currentPage-1);
          list_page(term,currentPage-1)
      }
  };

  const next_number = () =>{
    window.scrollTo(0, 0);
    console.log('test')
    if (currentPage !== Math.ceil(list.length/2)) {
      console.log('test')
      setCurrentPage(currentPage+1);
      setActive(currentPage+1);
      list_page(term,currentPage+1)
    }
  };

  let numbers = [];

  const getNumbers = () => {
    let itemsPerPage = listingsPerPage;
    let pageNumber = 1;

    for (let i = 0; i < count; i += itemsPerPage) {
      const page = pageNumber;
      let content = null;

      if (active === page) {
        content = (
            <div key={i} className={`hidden md:-mt-px md:flex`}>
                <div className="border-indigo-500 text-indigo-600 border-t-2 pt-4 px-4 inline-flex items-center text-sm font-medium">
                {pageNumber}
                </div>
            </div>
        );
      }
      else {
        content = (
            <div key={i} onClick={() => {
              visitPage(page)
              }} className={`hidden md:-mt-px md:flex`}>
              <div className="cursor-pointer border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 border-t-2 pt-4 px-4 inline-flex items-center text-sm font-medium">
                {pageNumber}
              </div>
            </div>
        );
      }

      numbers.push(
        content
      );
      pageNumber++;
    }

    return numbers;
  }




  
  return(
    <nav className="search-pagination border-t border-gray-200 px-4 flex items-center justify-between sm:px-0 mt-24">
        {
            currentPage !== 1 ?
            <div className="-mt-px w-0 flex-1 flex">
              <div
                onClick={()=>{previous_number()}}
                className="cursor-pointer pr-1 inline-flex items-center text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300"
              >
                <BellIcon className="mr-3 h-5 w-5 text-gray-400" aria-hidden="true" />
                Previous
              </div>
            </div>
            :

            <div className="-mt-px w-0 flex-1 flex">
              <div
                className="cursor-pointer pr-1 inline-flex items-center text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300">
              </div>
            </div>
        }
        {getNumbers()}
        { 
              numbers.length === 0 || currentPage === numbers.length ?

              <div className="-mt-px w-0 flex-1 flex justify-end">     
              </div>


              :
              <div className="-mt-px w-0 flex-1 flex justify-end">
                <button
                  onClick={()=>{next_number()}}
                  className="pl-1 inline-flex items-center text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300"
                >
                  Next
                  <BellIcon className="ml-3 h-5 w-5 text-gray-400" aria-hidden="true" />
                </button>
              </div>
              
            }
    </nav>
)
}

export default SmallSetPaginationSearch
