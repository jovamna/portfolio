...
function ResultRow({ label, value, textColorClass, large }) {
  return (
    <div className="flex justify-between items-center py-2">
      <p className={`text-sm ${large ? 'font-bold text-black' : 'text-black'}`}>{label}</p>
      <p className={`font-bold tracking-tight ${large ? 'text-xl text-gray-900' : textColorClass || 'text-black'}`}>
        {value}
      </p>
    </div>
  );
     ...
     {/* El icono se queda limpio y sin subrayar */}
      ...
      {result && (
        <div className="mt-2 p-4 bg-slate-50/70 rounded-xl border border-slate-100 flex flex-col gap-1">
          <ResultRow
            label="Coste ahora mismo:" className="font-bold text-black"
            value={fmtEur(result.costoNow)}
            large
          />
          <hr className="my-1.5 border-gray-200/60" />
          <ResultRow
            label="Con la hora más barata hoy:" className="font-bold text-black"
            value={fmtEur(result.costoMin)}
            textColorClass="text-green-600"
          />
          <ResultRow
            label="Con la hora más cara hoy:" className="font-bold text-black"
            value={fmtEur(result.costoMax)}
            textColorClass="text-red-500"
          />
          <hr className="my-1.5 border-gray-200/60" />
          <ResultRow
            label="Ahorro si esperas a la hora barata:" className="font-bold text-black"
            value={`+ ${fmtEur(Math.max(0, result.ahorro))}`}
            textColorClass="text-blue-600"
          />

          {/* Context note */}
          <p className="text-[11px] leading-relaxed text-gray-900 mt-3 text-center sm:text-left">
